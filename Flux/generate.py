...
async def main(_, msg):
    await msg.reply(
        ...
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("Pyrogram", callback_data="بايروجرام"),
            InlineKeyboardButton("Telethon", callback_data="تيليثون")
        ]])
    )


async def generate_session(bot, msg, telethon=False):
    await msg.reply("جارٍ بدء {} إنشاء الجلسة...".format("تيليثون" if telethon else "بايروجرام"))
    user_id = msg.chat.id
    api_id_msg = await bot.ask(user_id, 'الرجاء ارسال الخاص بك `API_ID`', filters=filters.text)
    ...
    try:
        ...
        return
    api_hash_msg = await bot.ask(user_id, ' الرجاء ارسال الخاص بك `API_HASH`', filters=filters.text)
    if await cancelled(api_id_msg):
        return
    api_hash = api_hash_msg.text
    phone_number_msg = await bot.ask(user_id, 'الآن يرجى إرسال "رقم حسابك" الخاص بك مع رمز الدوله.`رقم حسابك` \مثال : `+10000000000`', filters=filters.text)
    if await cancelled(api_id_msg):
        return
    phone_number = phone_number_msg.text
    await msg.reply("يتم الارسال الى تطبيق Telegram...")
    if telethon:
    ...
    try:
        ...
        return
    try:
        phone_code_msg = await bot.ask(user_id, "تم ارسال الرمز الى حسابك في Telegram. \n\nIf you got it, قم بارسالها بارقام متفرقه\nIf ❌ `12345`, **✅** `1 2 3 4 5`.", filters=filters.text, timeout=600)
        if await cancelled(api_id_msg):
            return
    except TimeoutError:
    ...
    try:
        ...
        try:
            two_step_msg = await bot.ask(user_id, 'حسابك مفعل عليها التحقق بخطوتين ارسل كلمة سر الحساب', filters=filters.text, timeout=300)
        except TimeoutError:
    ...
    text = "**{} سلسلة جلسة** \n\n`{}` \n\nتم إنشاؤها بواسطة : @H_M_Dr".format("تيليثون" if telethon else "بايروجرام", string_session)
    try:
        await client.send_message("me", text)
    except KeyError:
        pass
    await client.disconnect()
    await phone_code_msg.reply("تم إنشاء جلسة سلسلة {} بنجاح \n\nيرجى التحقق من رسائلك المحفوظة \n\nبواسطة : @H_M_Dr ".format("تيليثون" if telethon else "بايروجرام"))
